...
function manualResponseAccent(descriptor){
  if(!descriptor)return stepColor(0);
  if((descriptor.visualType==='read'||descriptor.visualType==='assign')&&descriptor.bindingName){
    return bindingIdentityColor(descriptor.bindingName,descriptor.bindingKind==='constant'?'constant':'variable');
  }
  return descriptor.operationColor||stepColor(0);
}
...
function requestManualResponse(descriptor,onConfirm){
  ...
  if(descriptor.isBoolean){
    const accent=manualResponseAccent(descriptor);
    const trueButton=h('button',{type:'button','data-value':'true','aria-pressed':'false'},'TRUE');
    const falseButton=h('button',{type:'button','data-value':'false','aria-pressed':'false'},'FALSE');
    control=h('div',{class:'manual-response-boolean','aria-label':'Choose a boolean value',style:`--manual-response-accent:${accent};`},trueButton,falseButton);
    [trueButton,falseButton].forEach(button=>button.onclick=()=>{
      [trueButton,falseButton].forEach(other=>{const selected=other===button;other.classList.toggle('selected',selected);other.setAttribute('aria-pressed',String(selected));});
